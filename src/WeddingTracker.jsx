...
export default function WeddingTracker() {
  ...
  // Как только кто-то из друзей меняет данные — всем прилетает обновление.
  useEffect(() => {
  ...
  const addTask = (block) => {
    const title = prompt(`Новая задача — «${block}»:`);
    if (!title) return;
  ...
  const { tasks, meetings, files } = data;
  ...
  const blocks = [...new Set(tasks.map(t => t.block))];
